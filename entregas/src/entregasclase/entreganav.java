	...
	public static void main(String[] args) {
		...
		try {
			FileWriter fichero = new FileWriter(
					"C:\\Users\\HP\\git\\repository\\entregas\\src\\entregasclase\\tareasacompletar", true); //Asigna el URL del archivo para guardar la infomacion
			Hashtable tareas = new Hashtable(); //Crea la tabla para agregar las tareas pendientes
			Hashtable tareascomp = new Hashtable(); //Crea la tabla para agregar las tareas completas

			System.out.println("Ingresar las tareas pendientes (hasta 5)"); //Pide y agrega las tareas que el usuario asigna como pendientes
			...
			PrintWriter pw = new PrintWriter(fichero); //Imprime las distintas tareas agregadas por el usuario en el archivo asignado previamente
			...
			switch (opcionqhacer) { //Da opciones al usuario segun lo elegido previamente
			...
			case 3:
				...
				if (opcionqhacer == 3) {
					...
					switch (opcionqeliminar) { //Desglose de las tareas a modificar para ponerlas el usuario en completas
					case 1:
			...
			pw.close(); //Cierre para imprimir en el archivo
